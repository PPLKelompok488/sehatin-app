    ...
    return (
        <AppLayout>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 mt-2 font-['Manrope']">
                ...
                {/* Jadwal Terdekat */}
                <div className="bg-primary rounded-[20px] p-6 flex flex-col justify-between shadow-md text-primary-foreground">
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="flex items-center gap-2 bg-white/10 px-2 py-1.5 rounded-full border border-white/20">
                            ...
                            <div className="text-[10px] font-semibold pr-2 text-white">
                    ...
                    <div>
                        ...
                        <h2 className="text-[22px] font-extrabold flex items-center gap-2 leading-tight text-white">{closestAppointment?.date || 'Tidak Ada'}</h2>
                ...
                <div className="bg-surface border border-outline-variant/30 rounded-[20px] p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="w-2 h-2 rounded-full bg-success mt-1.5 mr-1 ring-4 ring-success/15 hidden md:block"></div>
                    </div>
                    <div>
                    ...
                    filteredAppointments.map((app) => (
                        <div key={app.id} className="bg-surface border border-outline-variant/30 rounded-[20px] p-4 flex flex-col md:flex-row md:items-center justify-between gap-5 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center gap-4 w-[280px]">
                                <Avatar className="w-12 h-12 rounded-2xl border border-outline-variant/20 shadow-sm shrink-0">
                                    <AvatarImage src={`https://ui-avatars.com/api/?name=${encodeURIComponent(app.doctor_name)}&background=random&color=fff&size=150`} alt={app.doctor_name} />
                                    <AvatarFallback className="rounded-2xl text-sm font-bold bg-primary text-primary-foreground">
                                        {app.doctor_name.substring(0, 2).toUpperCase()}
                                    </AvatarFallback>
                                </Avatar>
                            ...
                            <div className="flex items-center md:gap-14 flex-1 text-xs font-bold text-on-surface-variant justify-between md:justify-start w-full md:w-auto">
                                ...
                                <div className={`flex items-center gap-1.5 text-[9px] bg-transparent md:mx-6 tracking-wide font-extrabold ${app.status === 'booked' ? 'text-primary' : 'text-success'}`}>
                                    <div className={`w-1 h-1 rounded-full ${app.status === 'booked' ? 'bg-primary animate-pulse' : 'bg-success'}`}></div>
                                    {app.status === 'booked' ? 'AKTIF' : 'SELESAI'}
                                </div>
                            </div>

                            <div className="shrink-0 flex justify-end w-full md:w-auto mt-2 md:mt-0">
                                <Button variant="secondary" size="sm" className="h-9 px-5 py-2 font-bold text-sm bg-surface-container/50 hover:bg-surface-container text-on-surface-variant rounded-xl flex items-center gap-1 transition-colors">
                                    Detail
            ...
            <div className="mt-8 mb-4 flex justify-center">
                <Button variant="outline" size="sm" className="h-9 px-4 text-[11px] font-bold text-on-surface border-outline-variant/60 hover:bg-surface-container/50 bg-white/50 shadow-sm flex items-center gap-1">
