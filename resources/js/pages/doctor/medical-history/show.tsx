        ...
        <AppLayout>
            ...
            <main className="max-w-7xl mx-auto pb-16 pt-8">
                ...
                <section className="mb-12">
                    <div className="bg-white rounded-xl p-8 md:p-10 border border-outline-variant/40 shadow-sm flex flex-col md:flex-row gap-10 items-center md:items-start relative overflow-hidden">
                        ...
                        <div className="flex-shrink-0">
                            <div className="w-32 h-32 rounded-xl overflow-hidden border-2 border-outline-variant/10 shadow-inner">
                                <Avatar className="w-full h-full rounded-none">
                                    <AvatarImage src={patient.avatar || ''} alt={patient.name} className="object-cover" />
                                    <AvatarFallback className="text-2xl bg-primary/10 text-primary rounded-none">
                        ...
                        <div className="flex-1">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-4">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-on-surface-variant mt-1">
                                        {patient.gender === 'L' ? 'Laki-laki' : 'Perempuan'}, {patient.age ? `${patient.age} Tahun` : '-'}
                                    </p>
                                </div>
                                <div className="flex flex-col gap-3 min-w-[200px]">
                                    <Link href={route('doctor.medical-record.create', appointment.id)}>
                                        <Button className="w-full bg-primary text-white font-semibold text-sm shadow-xl shadow-primary/20 flex items-center justify-center gap-2 hover:translate-y-[-2px] active:translate-y-0 transition-all py-3 rounded-xl">
                                            <span className="material-symbols-outlined text-lg">edit_note</span>
                                            Rekam Medis Pasien
                                        </Button>
                                    </Link>
                                    <Link href={route('doctor.medical-record.history', patient.id)}>
                                        <Button variant="outline" className="w-full border-primary/20 text-primary font-semibold text-sm hover:bg-primary/5 transition-all flex items-center justify-center gap-2 py-3 rounded-xl">
                            ...
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 mb-8 mt-4">
                                ...
                                <div className="flex items-center gap-3 pl-6 border-l border-outline-variant/30">
                                    <Avatar className="w-8 h-8 rounded-full border border-outline-variant/20 shadow-sm">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-sm font-semibold text-on-surface mt-1">{doctor.name}</span>
                                    </div>
                                </div>
                ...
                {medical_record ? (
                        ...
                        <div className="xl:col-span-8 space-y-8">
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <ClinicalCard letter="S" title="Subjektif" color="bg-[#E0F2FE] text-[#0284C7]">
                                        {medical_record.subjective}
                                    </ClinicalCard>
                                    <ClinicalCard letter="O" title="Objektif" color="bg-[#DCFCE7] text-[#166534]">
                                        {medical_record.objective}
                                    </ClinicalCard>
                                    <ClinicalCard letter="D" title="Diagnosis" color="bg-[#FEF3C7] text-[#92400E]">
                                        {medical_record.assessment}
                                    </ClinicalCard>
                                    <ClinicalCard letter="R" title="Rekomendasi" color="bg-[#F5F3FF] text-[#6D28D9]">
                                        {medical_record.plan}
                                    </ClinicalCard>
                                </div>
                            </div>
                            ...
                            <div className="mt-8">
                                ...
                                <div className="bg-white p-8 rounded-xl border border-outline-variant/10">
                        ...
                        <aside className="xl:col-span-4 space-y-8">
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <VitalCard icon="blood_pressure" label="Tekanan darah" value={medical_record.blood_pressure} normal="Normal: <120/80" iconColor="text-red-500" />
                                    <VitalCard icon="favorite" label="Detak jantung" value={`${medical_record.heart_rate} bpm`} normal="Normal: 60-100" iconColor="text-primary" />
                                    <VitalCard icon="device_thermostat" label="Suhu tubuh" value={`${medical_record.temperature}°C`} normal="Normal: 36.5-37.5" iconColor="text-amber-500" />
                                    <VitalCard icon="weight" label="Berat badan" value={`${medical_record.weight} kg`} normal="" iconColor="text-blue-400" />
                                    <div className="col-span-2">
                                        <VitalCard icon="water_drop" label="Gula darah" value={`${medical_record.blood_sugar} mg/dL`} normal="Normal: 70-110 mg/dL" iconColor="text-purple-500" />
                                    </div>
                                </div>
                            </div>
                        </aside>
                    ...
                    /* Empty State Content */
                    <div className="flex flex-col items-center justify-center py-32 px-6 bg-surface-container/30 border-2 border-dashed border-outline-variant rounded-3xl text-center">
...
function TimelineStep({ label, active }: { label: string; active: boolean }) {
    return (
        <div className="relative z-10 flex flex-col items-center gap-2 bg-white px-3">
            ...
            <span className={cn(
                "text-[10px] font-bold tracking-wider transition-colors duration-500 uppercase",
...
function ClinicalCard({ letter, title, color, children }: { letter: string; title: string; color: string; children: React.ReactNode }) {
    return (
        <div className="bg-white p-6 rounded-xl border border-outline-variant/20 hover:border-primary/30 transition-all group">
...
function VitalCard({ icon, label, value, normal, iconColor }: { icon: string; label: string; value: string; normal: string; iconColor: string }) {
    return (
        <div className="bg-white p-5 rounded-xl flex flex-col items-center text-center border border-outline-variant/20">
